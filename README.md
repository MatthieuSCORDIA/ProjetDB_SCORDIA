...
CLIENT = (Email_Client VARCHAR(50), Nom_Client VARCHAR(50), Prenom_Client VARCHAR(50), Date_Naissance DATE, Telephone BIGINT);
PRODUIT = (SKU_Produit VARCHAR(15), Code_Barre_EAN VARBINARY(13), Libelle_Produit VARCHAR(150), Description_Detaillee VARCHAR(500), Prix_de_vente_TTC DECIMAL(4,2), Categorie VARCHAR(30), Plateforme VARCHAR(20), Etat_Produit VARCHAR(10), Editeur_Marque VARCHAR(50), Date_Sortie_Officielle DATE);
MAGASIN = (Id_MAGASIN COUNTER, Nom_Magasin VARCHAR(50), Ville VARCHAR(50), Code_Postal BIGINT);
PRECOMMANDE = (#Email_Client, Num_COMMANDE COUNTER, Date_Precommande DATETIME, Montant_Acompte DOUBLE, #SKU_Produit);
CARTE_FIDELITE = (Num_Carte_Fidelite VARCHAR(20), Point_Cumules BYTE, Statu_Carte VARCHAR(15), #Email_Client);
RECOMMANDER = (#SKU_Produit, #SKU_Produit_1);
ACHETER = (#Email_Client, #SKU_Produit, #Id_MAGASIN, Num_Ticket_Caisse VARCHAR(20), Date_Transaction DATE, Heure_Transaction TIME, Quantite_Achetee INT, Montant_Total_Ticket DECIMAL(6,2));
STOCKER = (#SKU_Produit, #Id_MAGASIN, Quantite_en_stock INT);

Prompt d'insertion:
Rôle : Tu es un expert en bases de données SQL.

Contexte : Je développe une base de données pour un magasin de jeux vidéo. Voici la structure exacte de mes tables (MLD) :
CLIENT = (Email_Client VARCHAR(50), Nom_Client VARCHAR(50), Prenom_Client VARCHAR(50), Date_Naissance DATE, Telephone BIGINT);
PRODUIT = (SKU_Produit VARCHAR(15), Code_Barre_EAN VARBINARY(13), Libelle_Produit VARCHAR(150), Description_Detaillee VARCHAR(500), Prix_de_vente_TTC DECIMAL(4,2), Categorie VARCHAR(30), Plateforme VARCHAR(20), Etat_Produit VARCHAR(10), Editeur_Marque VARCHAR(50), Date_Sortie_Officielle DATE);
MAGASIN = (Id_MAGASIN COUNTER, Nom_Magasin VARCHAR(50), Ville VARCHAR(50), Code_Postal BIGINT);
PRECOMMANDE = (#Email_Client, Num_COMMANDE COUNTER, Date_Precommande DATETIME, Montant_Acompte DOUBLE, #SKU_Produit);
...
STOCKER = (#SKU_Produit, #Id_MAGASIN, Quantite_en_stock INT);
Instructions :
1. Génère les requêtes INSERT INTO pour remplir cette base.
2. Volume attendu :
10 clients (certains avec carte de fidélité, d'autres sans).
5 magasins.* 20 produits (mélange de jeux PS5, Switch, figurines et vêtements).
15 ventes dans la table ACHETER.
Le stock correspondant pour chaque magasin.3. Cohérence : Respecte scrupuleusement les contraintes d'intégrité (les IDs doivent correspondre entre les tables).
Format : Donne-moi uniquement le code SQL

scénario d'utilsation: le Directeur Régional de Micromania supervise plusieurs points de vente physiques. Il doit optimiser la rotation des stocks, identifier les produits à fort potentiel et récompenser la fidélité pour augmenter le chiffre d'affaires de la zone. L'objectif est d'extraire des données précises pour préparer le comité de direction mensuel et ajuster les campagnes promotionnelles.
